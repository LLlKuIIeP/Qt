...
SyntaxHighligher::SyntaxHighligher(QTextDocument *parent) : QSyntaxHighlighter(parent) {
    m_lstKeywords << "foreach"  << "bool"   << "int"    << "void"   << "double"
                  << "float"    << "char"   << "delete" << "class"  << "const"
                  << "virtual"  << "union"  << "this"   << "struct" << "mutable"
                  << "throw"    << "for"    << "if"     << "else"   << "private"
                  << "switch"   << "case"   << "public" << "false"  << "namespase"
                  << "new"      << "return" << "using"  <<  "true"  << "protected"
                  << "->"       << ">>"     << "<<"     << ">"      << "<"
                  << "("        << ")"      << "{"      << "}"      << "["
                  << "]"        << "+"      << "-"      << "*"      << "/"
                  << "="        << "!"      << "."      << ","      << ";"
...
/*virtual*/ void SyntaxHighligher::highlightBlock(const QString &str) {
    int nState = previousBlockState();  //текущее состояние хранится в nState, получаем его из previousBlockState()
    int nStart = 0;
...
QString SyntaxHighligher::getKeyword(int nPos, const QString &str) const {
    QString strTemp = "";
