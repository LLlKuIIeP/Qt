...
void SelectFile::slot_select_file(QString f) {
    //m_input_file = f.replace(QString("/"), QString("\\"));    //windows
    m_input_file = f;   //linux
    qDebug() << "SelectFile: " + m_input_file;
    emit signsl_selected_file(this);
}

void SelectFile::slot_save_file(QString s) {
    //m_save_file = m_input_file.left(m_input_file.lastIndexOf('\\') + 1) + s + ".txt";//windows
    m_save_file = m_input_file.left(m_input_file.lastIndexOf('/') + 1) + s + ".txt";//linux
